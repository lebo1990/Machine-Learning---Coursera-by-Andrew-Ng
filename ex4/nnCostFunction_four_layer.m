function [J grad] = nnCostFunction_four_layer(nn_params, ...
                                   input_layer_size, ...
                                   hidden_layer_size, ...
                                   num_labels, ...
                                   X, y, lambda)
...
Theta3 = reshape(nn_params(hidden_layer_size * (hidden_layer_size + 1) + hidden_layer_size * (input_layer_size + 1) + 1 : end), ...
...
J = 1.0 / m * (-sum(sum((y_new .* log(a4) + (1 - y_new) .* log(1 - a4)))) + ...
	lambda / 2.0 * (sum(sum(Theta1(:, 2: end) .^ 2)) + ...
					sum(sum(Theta2(:, 2: end) .^ 2)) + ...
					sum(sum(Theta3(:, 2: end) .^ 2))));

delta4 = a4 - y_new;

delta3 = delta4 * Theta3 .*sigmoidGradient([zeros(size(z3, 1), 1) ,z3]);
delta3 = delta3(:, 2:end);
